...
| **Document** | **Details** |
|---|---|
| **API baseline** | Cymulate OpenAPI 2.0.62 |
| **Runtime** | PowerShell 7 + Windows Task Scheduler |
| **Updated** | August 19, 2026 |

> **Design in one sentence:** Cymulate owns execution and source evidence; Jira owns operational workflow and `Blue team detection`; VECTR owns technical test history and defense outcomes; local durable state coordinates synchronization and recovery.

### Current POC design decisions

- The launched-assessment scenario result `detection` field (`data[].detection` from `GET /v2/assessments/launched/{id}/scenarios`) is the documented scenario-level Cymulate detection source.
- `GET /v2/assessments/launched/{id}/findings` supplies supporting prevention/control evidence plus AV, EDR, and TheHive use-case evidence.
- Alternate Cymulate findings/search/detail APIs are outside the reporting design. Unresolved evidence routes to `MANUAL_REVIEW_REQUIRED` and Jira `Analyze` rather than causing another evidence query or a fabricated verdict.
- Proven AV/EDR prevention maps to Jira `Blocked`; AV, EDR, and TheHive evidence remain separately preserved.
- The same normalized evidence renders both Jira `Blue team detection` and the VECTR Test Case overall defense outcome. Attributable per-tool VECTR outcomes are written when supported.

> **Terminology:** “scenario result `detection` field” means the `detection` property on each item returned by `GET /v2/assessments/launched/{id}/scenarios`. It is a response field, not a separate endpoint.
...
- Be testable phase-by-phase with Pester and synthetic API responses before live Jira/VECTR writes are enabled.
...
- Invent a finding-to-scenario correlation rule that has not been validated against actual Cymulate API responses from the organization's environment.
...
Any write-enabled POC or pilot Jira automation must use a dedicated non-person account.

Recommended display name:

**Application name:** `Purple Team Orchestrator`
...
- **Project:** `CIBETHICAL`
- **Issue key:** `CIBETHICAL-####` — generated by Jira during the issue-creation POST.
...
**Recommended automated summary:** `[Purple Team] <Scenario Name> - <YYYY-MM-DD>`

The summary is human-readable and must **not** be treated as the primary idempotency/recovery key.

**Recommended correlation fields:** `Purple Run ID`, `Cymulate Assessment ID`, `Cymulate Scenario Result ID`, `Cymulate Scenario ID`, `VECTR Test Case ID`, `Stakeholder`, `Run Type`, and `Target`.
...
- Store the Jira custom-field ID and option IDs in configuration after discovery against actual Jira project metadata.
...
3. Scenario `detection` (`Detected`, `Not Detected`, empty, or unknown).
...
6. If a later scheduled invocation or human-authoritative resolution changes the substantive result, update and read back the VECTR outcome as well as the Jira verdict where automation still owns the field. Phase 0 must prove the exact `testCase.update` contract used for post-create overall-outcome changes before write-enabled automation relies on it.
...
This includes `BLUE_TEAM_ANALYZE`, `DONE`, `TODO_PCSIRT`, `TODO_PURPLE`, and `TODO_PCSIRT_LATER`.
...
- Cymulate token should be read-only for the POC wherever permission controls allow it.
...
| Wrong Jira value or workflow transition | High | Exact allowlist, validation against actual Jira options, decision tests, read-back, transition-by-name, Jira Automation idempotency. |
| Human edits are overwritten | Medium | Field ownership model + conflict detection. |
| Personal Jira account is used by automation | High | Dedicated service account; startup validation in any write-enabled environment; no fallback to personal credentials. |
...
- Supplied Cymulate OpenAPI Swagger (`swagger.json`), OpenAPI 3.1.1, Cymulate API version 2.0.62, reviewed August 18, 2026. This is the primary source for overlapping endpoint/schema contracts in this draft.
...
- Jira REST API documentation and actual project field/workflow metadata.
...
- PowerShell 7 only for implementation code.
- Pester tests accompany every behavior change.
- No Cymulate mutation calls in the POC.
...
**Goal:** Prove the actual Cymulate/Jira/VECTR contract and create the PowerShell repository without live Jira/VECTR automation writes.
...
| P3-T11 | Add negative tag and deprecated-outcome tests. | `tests/Unit/Vectr.Tags.Tests.ps1`, outcome tests | Implementation module performs zero VECTR tag mutation and never uses deprecated create-time `outcome` as the primary outcome writer. | S |
...
### Phase 7: Shadow Mode, UAT & Controlled POC Pilot

**Goal:** Prove the automation against the existing human process in a narrowly controlled POC before any broader rollout.
...
| P7-T6 | Enable writes for a narrowly defined POC pilot launched-assessment scope (for example approved assessment/template IDs, tags, names, or environments). Process every scenario result inside that scope. | pilot config | ≥20 successful pilot ScenarioRuns; 0 scenario results silently skipped because an override is absent; 0 duplicate Jira; 0 duplicate VECTR; 0 VECTR tag mutations. | XL |
| P7-T7 | POC acceptance review. | `docs/poc-acceptance.md` | All P0/P1 reliability targets met; rollback, credential rotation, and operator runbook reviewed; decision recorded on whether to proceed beyond the POC. | M |
...
### Phase 8: Post-POC Production Rollout (Future)

**Goal:** If the POC is accepted, expand the proven assessment-driven workflow into normal production operation without introducing a scenario-ID allowlist. This phase is outside POC acceptance.

> **Future phase:** Phase 8 is included to show the likely path from a successful POC to broader operation. It is not required to declare the POC successful.
...
| P8-T2 | Expand from pilot assessment/template scope to the remaining approved production discovery scope in controlled batches. | rollout config | Every scenario result inside each enabled assessment scope is discovered and tracked exactly once. | M |
...
|-------|------|-------|------------|-----------------|
| P0 | Contract Discovery & Scaffolding | 7 | — | Proven actual API/workflow mappings |
...
| P7 | Shadow/UAT/POC Pilot | 7 | P6 | POC acceptance evidence |
| P8 | Post-POC Production Rollout (Future) | 5 | P7 | Broad assessment-driven rollout after POC approval |

**POC scope: 61 tasks across Phases 0–7. Post-POC rollout: 5 future tasks in Phase 8.**
...
- Implementation functions use approved PowerShell verb-noun naming.
...
Post-POC roadmap candidates, considered only after workflow integrity is proven:
...
This addendum is explanatory. Parts I and II remain normative if a diagram and a requirement ever conflict.
...
PowerShell Orchestrator
     ...
     Cymulate
        ...
        │
        ▼
Evidence normalization + verdict engine
        │
        ├───────────────┐
        ▼               ▼
      Jira             VECTR
  workflow/status   technical history
  Blue team         overall outcome
  detection         per-tool outcomes
  comments          evidence notes
        │               │
        └───────┬───────┘
                ▼
        Local durable state
     idempotency · retries · audit
           reconciliation
```

**System-of-record boundary:** Cymulate supplies source evidence, Jira records the operational verdict/workflow, VECTR records the technical test result, and local state coordinates the automation.

### A.2 Scheduled Discovery and Lookback

```text
last successful discovery checkpoint
        │
        ├─ subtract configured overlap
        ▼
...
for each relevant assessment
        │
        ├─ non-terminal → persist pending state → revisit next run
        │
        └─ terminal ───→ enumerate every scenario result
                              │
                              ▼
                      process each result
                      (no scenario-ID allowlist)
                              │
                              ▼
                       persist safely
                              │
...
The overlap is intentional. Duplicate safety comes from the idempotency key, not from a perfectly non-overlapping discovery window.
...
1 launched assessment
        │
        ├─ scenario result 1 ─→ Jira issue 1 + VECTR Test Case 1
        ├─ scenario result 2 ─→ Jira issue 2 + VECTR Test Case 2
        ├─ scenario result 3 ─→ Jira issue 3 + VECTR Test Case 3
        └─ ...
```

**Rule:** one Cymulate scenario result is one reporting unit. A ten-scenario assessment can therefore produce ten Jira/VECTR record pairs.

### A.4 Evidence to Verdict

```text
Cymulate scenario result
        │
        ├─ execution status
        ├─ scenario detection
        ├─ AV evidence
        ├─ EDR evidence
        └─ TheHive use-case evidence
        │
        ▼
normalize each evidence dimension
        │
        ▼
apply precedence rules
        │
        ├─ execution failed ─────────────→ AttackFailed
        ├─ partial execution ────────────→ PartialTesting
        ├─ AV/EDR prevented activity ───→ Blocked
        ├─ expected TheHive use case fired → Detected
        ├─ confirmed use-case miss ──────→ NotDetected
        ├─ detection genuinely N/A ──────→ NotApplicable
        └─ evidence unresolved/conflicting → MANUAL_REVIEW_REQUIRED
```

The **same normalized evidence set** drives both the Jira result and the VECTR outcome. There is no separate VECTR verdict engine.

### A.5 Jira Result vs. Workflow State

```text
                    Can evidence support
        ...
        set Blue team detection   leave field unset
                   │                 │
                   ▼                 ▼
            read-back verify      MANUAL_REVIEW_REQUIRED
                   │                 │
                   ▼                 ▼
          normal Jira routing       Analyze
```

Active `Blue team detection` values:

`NotApplicable` · `NotDetected` · `Detected` · `NotStarted` · `Blocked` · `AttackFailed` · `PartialTesting` · `TestingInProgress`

Manual review is a **workflow condition**, not another detection verdict.

### A.6 Recommended VECTR Organization
...
└─ Assessment: Cymulate Continuous Validation — 2026 Q3
   ...
   └─ Campaign: Other approved grouping
      └─ [CIBETHICAL-1845] Scenario D
```

Cymulate-originated Test Cases should normally stay in the VECTR Environment representing the defensive environment being tested. A separate VECTR Environment/database is warranted only for a real isolation boundary such as different infrastructure, Defense Tools, access requirements, governance, or reporting ownership.

For the POC, the required VECTR Assessments and Campaigns are **administrative prerequisites**. The orchestrator resolves and uses their approved IDs; it does not create them dynamically.

### A.7 VECTR Outcomes

```text
one Cymulate scenario result
        │
        ▼
...
one VECTR Test Case
        │
        ├─ overall Test Case outcome
        ├─ AV per-tool outcome       (when attributable)
        ├─ EDR per-tool outcome      (when attributable)
        ├─ TheHive per-tool outcome  (when attributable)
        └─ compact outcomeNotes
```

Cymulate findings are supporting evidence for the Test Case; they do **not** become separate VECTR Test Cases.

If VECTR's per-tool priority would derive an overall outcome that conflicts with the approved Purple Team verdict, the orchestrator applies the approved overall-outcome override and verifies the stored result.

For unresolved/manual-review cases, VECTR uses the approved `TBD` outcome until the case is resolved.

### A.8 Correlation and Idempotency
...
external_key = <assessmentId>:<scenarioResultID>
        │
        ▼
local ScenarioRun + internal run_id
        │
        ▼
create/recover Jira
        │
        ▼
Jira generates CIBETHICAL-####
        │
        ▼
persist jira_key
        │
        ▼
create/recover VECTR Test Case
        │
        ▼
read back Jira + VECTR and verify
```

If a create response is lost, the script **recovers the existing object instead of blindly creating again**. Jira recovery uses the persisted run correlation; VECTR recovery uses the deterministic Test Case identity and verifies the expected Campaign/library mapping before reuse.

### A.9 Reconciliation and Completion Barrier

```text
Cymulate result is terminal
        │
        ▼
evidence resolved
        │
        ▼
Jira desired state written + verified
        │
        ▼
VECTR desired state written + verified
        │
        ▼
all automation-owned state matches?
        │
        ├─ yes → COMPLETE
        │
        └─ no  → SYNC_PENDING / retry / conflict
                         │
                         ▼
                 next scheduled run
                         │
...
Cymulate completion alone never marks the orchestration run complete. A manual-review run may be synchronized from an automation perspective while remaining open in Jira for analyst work.

### A.10 Operator Summary

1. Task Scheduler starts one finite PowerShell run.
2. The script searches the configured launched-assessment lookback window.
3. Every scenario result in each in-scope assessment is considered; no per-scenario enrollment is required.
4. Cymulate status/detection plus AV, EDR, and TheHive evidence feed one normalized evidence model.
5. One scenario result produces one Jira issue and one VECTR Test Case.
6. Jira records the substantive `Blue team detection` verdict and workflow state.
7. VECTR records the overall technical outcome, attributable per-tool outcomes, and compact evidence notes.
8. Local durable state prevents duplicates and enables retry/recovery.
9. Jira and VECTR writes are read back and verified.
10. The run is complete only when all automation-owned state is synchronized.
